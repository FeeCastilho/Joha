...
(function(){
  ...
  if(rot && !reduce){
    const words = ['tratamento capilar','saúde do couro cabeludo','Head Spa','cortes com essência'];
    let i = 0;
    setInterval(()=>{
      i = (i+1) % words.length;
  ...
  /* ---- scroll reveals ---- */
  const revealSel = '.manifesto .leaf-rule, .manifesto p, #pilares .sec-head, .pillar, .prices .head-row, .price-item, .price-note, .spa .ph, .spa .eyebrow, .spa h2, .spa p, .spa .btn, #equipe .sec-head, .member, #visite .ph, #visite .eyebrow, #visite h2, #visite .info-row, .ig .eyebrow, .ig h2, .ig p, .ig .ph, .ig .btn, .cta-final .seal, .cta-final h2, .cta-final p, .cta-final > .wrap > div:last-child';
